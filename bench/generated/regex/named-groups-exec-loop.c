...
main(void)
{
	static const uint8_t pattern_utf8[] =
			"(?<word>[a-z]+)(?<digits>\\d+)";
	...
	for (i = 0; i < 20000u; i++) {
		...
		for (;;) {
			...
			checksum += len;
			if (jsval_object_get_utf8(&region, match,
					(const uint8_t *)"groups", 6, &groups) < 0) {
				return bench_fail_errno("jsval_object_get_utf8(groups)");
			}
			if (jsval_object_get_utf8(&region, groups,
					(const uint8_t *)"word", 4, &value) < 0) {
				return bench_fail_errno("jsval_object_get_utf8(groups.word)");
			}
			if (jsval_string_copy_utf8(&region, value, NULL, 0, &len) < 0) {
				return bench_fail_errno("jsval_string_copy_utf8(groups.word)");
			}
			checksum += len;
			if (jsval_object_get_utf8(&region, groups,
					(const uint8_t *)"digits", 6, &value) < 0) {
				return bench_fail_errno("jsval_object_get_utf8(groups.digits)");
			}
			if (jsval_to_number(&region, value, &number) < 0) {
				return bench_fail_errno("jsval_to_number(groups.digits)");
			}
