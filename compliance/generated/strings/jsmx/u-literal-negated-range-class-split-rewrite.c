...
#define CASE_NAME "jsmx/u-literal-negated-range-class-split-rewrite"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(jsval_method_string_split_u_literal_negated_range_class(
			...
			SUITE, CASE_NAME, "negated range split failed");
	GENERATED_TEST_ASSERT(result.kind == JSVAL_KIND_ARRAY
			&& jsval_array_length(&region, result) == 3,
			SUITE, CASE_NAME, "expected three negated range split parts");
	GENERATED_TEST_ASSERT(jsval_array_get(&region, result, 0, &value) == 0,
			SUITE, CASE_NAME, "failed to fetch negated range split[0]");
	GENERATED_TEST_ASSERT(jsval_string_new_utf16(&region, pair_units,
			sizeof(pair_units) / sizeof(pair_units[0]), &expected_value) == 0
			&& jsval_strict_eq(&region, value, expected_value) == 1,
			SUITE, CASE_NAME, "unexpected negated range split prefix");
	GENERATED_TEST_ASSERT(jsval_array_get(&region, result, 1, &value) == 0,
			SUITE, CASE_NAME, "failed to fetch negated range split[1]");
	GENERATED_TEST_ASSERT(jsval_string_new_utf16(&region, b_d_low_units,
			sizeof(b_d_low_units) / sizeof(b_d_low_units[0]), &expected_value) == 0
			&& jsval_strict_eq(&region, value, expected_value) == 1,
			SUITE, CASE_NAME, "unexpected negated range split middle");
	...
	GENERATED_TEST_ASSERT(jsval_method_string_split_u_literal_negated_range_class(
			...
			&result, &error) == 0,
			SUITE, CASE_NAME, "negated range split with limit failed");
	GENERATED_TEST_ASSERT(result.kind == JSVAL_KIND_ARRAY
			&& jsval_array_length(&region, result) == 2,
			SUITE, CASE_NAME, "expected two negated range split parts with limit");
