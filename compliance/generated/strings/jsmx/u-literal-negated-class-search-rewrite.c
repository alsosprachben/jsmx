...
#define CASE_NAME "jsmx/u-literal-negated-class-search-rewrite"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(
			jsval_method_string_search_u_literal_negated_class(&region,
			class_subject, low_d_members,
			sizeof(low_d_members) / sizeof(low_d_members[0]), &result,
			&error) == 0,
			SUITE, CASE_NAME, "negated class search failed");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, result,
			jsval_number(3.0)) == 1,
			SUITE, CASE_NAME, "expected first negated class hit at 3");
	...
	GENERATED_TEST_ASSERT(
			...
			&result, &error) == 0,
			SUITE, CASE_NAME, "pair-only negated class search failed");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, result,
			jsval_number(-1.0)) == 1,
			SUITE, CASE_NAME,
			"negated class search should not match inside a pair");
	...
	GENERATED_TEST_ASSERT(
			...
			&error) == 0,
			SUITE, CASE_NAME, "isolated high-surrogate negated search failed");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, result,
			jsval_number(4.0)) == 1,
			SUITE, CASE_NAME, "expected isolated high-surrogate miss-set hit at 4");
