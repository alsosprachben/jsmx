...
#define CASE_NAME "jsmx/u-literal-class-replace-rewrite"
...
static int
class_replace_callback(jsval_region_t *region, void *opaque,
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(jsval_method_string_replace_u_literal_class(
			&region, class_subject, low_d_members,
			sizeof(low_d_members) / sizeof(low_d_members[0]), ascii_x,
			&result, &error) == 0,
			SUITE, CASE_NAME, "class replace failed");
	GENERATED_TEST_ASSERT(expect_utf16(&region, result, replace_first_expected,
			...
			SUITE, CASE_NAME, "class replace result mismatch");

	GENERATED_TEST_ASSERT(jsval_method_string_replace_all_u_literal_class(
			&region, class_subject, low_d_members,
			sizeof(low_d_members) / sizeof(low_d_members[0]), ascii_x,
			&result, &error) == 0,
			SUITE, CASE_NAME, "class replaceAll failed");
	GENERATED_TEST_ASSERT(expect_utf16(&region, result, replace_all_expected,
			...
			SUITE, CASE_NAME, "class replaceAll result mismatch");

	GENERATED_TEST_ASSERT(jsval_method_string_replace_u_literal_class(
			&region, subst_subject, low_d_members,
			sizeof(low_d_members) / sizeof(low_d_members[0]),
			special_replacement, &result, &error) == 0,
			SUITE, CASE_NAME, "class replacement with substitutions failed");
	GENERATED_TEST_ASSERT(expect_utf16(&region, result, special_expected,
			sizeof(special_expected) / sizeof(special_expected[0])) == 0,
			SUITE, CASE_NAME, "class substitution result mismatch");

	GENERATED_TEST_ASSERT(jsval_method_string_replace_all_u_literal_class_fn(
			&region, class_subject, low_d_members,
			sizeof(low_d_members) / sizeof(low_d_members[0]),
			class_replace_callback, &callback_ctx, &result,
			&error) == 0,
			SUITE, CASE_NAME, "class callback replaceAll failed");
	...
	GENERATED_TEST_ASSERT(expect_utf16(&region, result, callback_expected,
			...
			SUITE, CASE_NAME, "class callback replaceAll result mismatch");
