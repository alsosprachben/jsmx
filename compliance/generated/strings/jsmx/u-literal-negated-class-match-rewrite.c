...
#define CASE_NAME "jsmx/u-literal-negated-class-match-rewrite"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(
			...
			&error) == 0,
			SUITE, CASE_NAME, "global negated class match failed");
	GENERATED_TEST_ASSERT(result.kind == JSVAL_KIND_ARRAY
			&& jsval_array_length(&region, result) == 1,
			SUITE, CASE_NAME, "expected one global negated class match");
	GENERATED_TEST_ASSERT(jsval_array_get(&region, result, 0, &value) == 0
			&& expect_utf16(&region, value, b_unit, 1) == 0,
			SUITE, CASE_NAME, "global negated class match mismatch");

	GENERATED_TEST_ASSERT(
			jsval_method_string_match_all_u_literal_negated_class(&region,
			class_subject, low_d_members,
			sizeof(low_d_members) / sizeof(low_d_members[0]), &iterator,
			&error) == 0,
			SUITE, CASE_NAME, "negated class matchAll helper failed");
	GENERATED_TEST_ASSERT(iterator.kind == JSVAL_KIND_MATCH_ITERATOR,
			SUITE, CASE_NAME, "expected negated class matchAll iterator");
	GENERATED_TEST_ASSERT(jsval_match_iterator_next(&region, iterator, &done,
			&result, &error) == 0 && !done
			&& result.kind == JSVAL_KIND_OBJECT,
			SUITE, CASE_NAME, "expected first negated class matchAll result");
	GENERATED_TEST_ASSERT(jsval_object_get_utf8(&region, result,
			(const uint8_t *)"0", 1, &value) == 0
			&& expect_utf16(&region, value, b_unit, 1) == 0,
			SUITE, CASE_NAME, "negated class matchAll capture mismatch");
	GENERATED_TEST_ASSERT(jsval_object_get_utf8(&region, result,
			(const uint8_t *)"index", 5, &value) == 0
			&& jsval_strict_eq(&region, value, jsval_number(3.0)) == 1,
			SUITE, CASE_NAME, "negated class matchAll index mismatch");
	GENERATED_TEST_ASSERT(jsval_match_iterator_next(&region, iterator, &done,
			&result, &error) == 0 && done
			&& result.kind == JSVAL_KIND_UNDEFINED,
			SUITE, CASE_NAME, "negated class matchAll iterator should exhaust");
