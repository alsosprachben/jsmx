...
#define CASE_NAME "jsmx/u-literal-class-split-rewrite"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(jsval_array_get(&region, result, 2, &value) == 0
			&& expect_utf16(&region, value, high_c_units,
				sizeof(high_c_units) / sizeof(high_c_units[0])) == 0,
	...
	GENERATED_TEST_ASSERT(jsval_method_string_split_u_literal_class(
			...
			&result, &error) == 0,
			SUITE, CASE_NAME, "class split(limit) failed");
	GENERATED_TEST_ASSERT(result.kind == JSVAL_KIND_ARRAY
