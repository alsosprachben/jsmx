...
#define CASE_NAME "test262/substr/start-negative"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(generated_expect_accessor_string(&out,
			...
			"substr(-3)") == GENERATED_TEST_PASS,
			SUITE, CASE_NAME, "unexpected substr(-3)");

	GENERATED_TEST_ASSERT(jsmethod_string_substr(&out,
			jsmethod_value_string_utf8((const uint8_t *)"abc", 3), 1,
			jsmethod_value_number(-4.0), 0, jsmethod_value_undefined(),
			&error) == 0, SUITE, CASE_NAME, "failed substr(-4)");
	GENERATED_TEST_ASSERT(generated_expect_accessor_string(&out,
			(const uint16_t[]){'a', 'b', 'c'}, 3, SUITE, CASE_NAME,
			"substr(-4)") == GENERATED_TEST_PASS,
			SUITE, CASE_NAME, "unexpected substr(-4)");

	GENERATED_TEST_ASSERT(jsmethod_string_substr(&out,
			jsmethod_value_string_utf8((const uint8_t *)"abc", 3), 1,
			jsmethod_value_number(-1.1), 0, jsmethod_value_undefined(),
			&error) == 0, SUITE, CASE_NAME, "failed substr(-1.1)");
	GENERATED_TEST_ASSERT(generated_expect_accessor_string(&out,
			(const uint16_t[]){'c'}, 1, SUITE, CASE_NAME,
			"substr(-1.1)") == GENERATED_TEST_PASS,
			SUITE, CASE_NAME, "unexpected substr(-1.1)");
