...
#define CASE_NAME "test262/substr/this-non-obj-coerce"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(jsmethod_string_substr(&out,
			jsmethod_value_undefined(), 0, jsmethod_value_undefined(), 0,
	...
	GENERATED_TEST_ASSERT(error.kind == JSMETHOD_ERROR_TYPE,
			SUITE, CASE_NAME, "expected TYPE for undefined receiver, got %d",
			(int)error.kind);
	GENERATED_TEST_ASSERT(jsmethod_string_substr(&out,
			jsmethod_value_null(), 0, jsmethod_value_undefined(), 0,
			jsmethod_value_undefined(), &error) == -1,
