...
#define CASE_NAME "test262/split/arguments-are-regexp-s-and-3-and-instance-is-string-a-b-c-de-f"
...
main(void)
{
	 ...
	 * flattenable primitive string value while preserving the regex split limit.
	 */
	...
	GENERATED_TEST_ASSERT(jsval_method_string_split(&region, text, 1, regex, 1,
			jsval_number(3.0), &result, &error) == 0,
			SUITE, CASE_NAME, "regex split with limit failed");
	GENERATED_TEST_ASSERT(generated_expect_split_array_strings(&region, result,
			...
			GENERATED_TEST_PASS,
			SUITE, CASE_NAME, "regex split with limit mismatch");
