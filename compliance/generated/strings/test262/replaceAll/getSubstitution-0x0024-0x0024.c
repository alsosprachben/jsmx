...
#define CASE_NAME "test262/replaceAll/getSubstitution-0x0024-0x0024"
...
main(void)
{
	...
	static const uint8_t ninguem_utf8[] = "ninguém";
	static const uint8_t e_utf8[] = "é";
	...
	GENERATED_TEST_ASSERT(generated_measure_and_replace_all(&out, storage,
			...
			jsmethod_value_string_utf8(ninguem_utf8,
					sizeof(ninguem_utf8) - 1),
	...
	GENERATED_TEST_ASSERT(generated_measure_and_replace_all(&out, storage,
			GENERATED_LEN(storage), subject,
			jsmethod_value_string_utf8(e_utf8, sizeof(e_utf8) - 1),
			jsmethod_value_string_utf8((const uint8_t *)"$$", 2),
			&error) == 0, SUITE, CASE_NAME,
			"failed $$ replacement for \"é\"");
	GENERATED_TEST_ASSERT(generated_expect_utf8_string(&out,
			"Ningu$m $ igual a ningu$m. Todo o ser humano $ um estranho ímpar.",
			SUITE, CASE_NAME, "\"é\" -> \"$$\"") == GENERATED_TEST_PASS,
			SUITE, CASE_NAME, "unexpected $$ replacement for \"é\"");
	...
	GENERATED_TEST_ASSERT(generated_measure_and_replace_all(&out, storage,
			GENERATED_LEN(storage), subject,
			jsmethod_value_string_utf8(e_utf8, sizeof(e_utf8) - 1),
			jsmethod_value_string_utf8((const uint8_t *)"$$&", 3),
			&error) == 0, SUITE, CASE_NAME,
			"failed \"$$&\" replacement");
	GENERATED_TEST_ASSERT(generated_expect_utf8_string(&out,
			"Ningu$&m $& igual a ningu$&m. Todo o ser humano $& um estranho ímpar.",
