...
#define CASE_NAME "test262/codePointAt/returns-undefined-on-position-equal-or-more-than-size"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(jsmethod_string_code_point_at(&has_value, &value,
			...
			"failed codePointAt(3)");
	GENERATED_TEST_ASSERT(has_value == 0, SUITE, CASE_NAME,
			"expected undefined for codePointAt(3)");
	GENERATED_TEST_ASSERT(jsmethod_string_code_point_at(&has_value, &value,
			jsmethod_value_string_utf8((const uint8_t *)"abc", 3), 1,
			jsmethod_value_number(4.0), &error) == 0, SUITE, CASE_NAME,
			"failed codePointAt(4)");
	GENERATED_TEST_ASSERT(has_value == 0, SUITE, CASE_NAME,
			"expected undefined for codePointAt(4)");
	GENERATED_TEST_ASSERT(jsmethod_string_code_point_at(&has_value, &value,
			jsmethod_value_string_utf8((const uint8_t *)"abc", 3), 1,
