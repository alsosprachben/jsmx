...
#define SUITE "values"
#define CASE_NAME "test262/S11.11.1_A4_T3"
...
main(void)
{
	 ...
	 * cases. The String-object cases are intentionally omitted above the
	...
	GENERATED_TEST_ASSERT(jsval_string_new_utf8(&region, (const uint8_t *)"0", 1,
			...
			"failed to construct \"0\" string operand");
	...
	result = generated_logical_and(&region, zero_string, minus_one_string);
	GENERATED_TEST_ASSERT(generated_expect_value_string(&region, result,
			(const uint8_t *)"-1", 2, SUITE, CASE_NAME,
			"\"0\" && \"-1\"") == GENERATED_TEST_PASS,
			SUITE, CASE_NAME, "unexpected result for \"0\" && \"-1\"");

	result = generated_logical_and(&region, minus_one_string, x_string);
	GENERATED_TEST_ASSERT(generated_expect_value_string(&region, result,
			(const uint8_t *)"x", 1, SUITE, CASE_NAME,
