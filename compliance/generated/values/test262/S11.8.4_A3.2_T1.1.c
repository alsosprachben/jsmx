...
#define SUITE "values"
#define CASE_NAME "test262/S11.8.4_A3.2_T1.1"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(jsval_greater_equal(&region, one_string, one_string,
	...
	GENERATED_TEST_ASSERT(jsval_greater_equal(&region, x_string, one_string,
			&result) == 0, SUITE, CASE_NAME,
			"failed to lower \"x\" >= \"1\"");
	GENERATED_TEST_ASSERT(generated_expect_boolean_result(result, 1, SUITE,
			CASE_NAME, "\"x\" >= \"1\"") == GENERATED_TEST_PASS,
			SUITE, CASE_NAME, "unexpected result for \"x\" >= \"1\"");
	GENERATED_TEST_ASSERT(jsval_greater_equal(&region, one_string, x_string,
			&result) == 0, SUITE, CASE_NAME,
			"failed to lower \"1\" >= \"x\"");
