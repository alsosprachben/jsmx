...
#define CASE_NAME "jsmx/compression-stream-gzip-round-trip-parity"
...
main(void)
{
	static const uint8_t input[] = "Hello, streams!";
	...
	GENERATED_TEST_ASSERT(
			jsval_readable_stream_new_from_bytes(&region, input, input_len,
				&readable) == 0 &&
			...
			jsval_readable_stream_get_reader(&region, out_readable, &reader)
				== 0,
			SUITE, CASE_NAME,
			"readable + compression stream + pipeThrough");

	GENERATED_TEST_ASSERT(
			collect_bytes(&region, reader, compressed, sizeof(compressed),
				&compressed_len) == 0,
			SUITE, CASE_NAME, "collect compressed bytes");

	GENERATED_TEST_ASSERT(
			compressed_len >= 2 &&
			compressed[0] == 0x1F && compressed[1] == 0x8B,
			SUITE, CASE_NAME,
			"compressed output must begin with gzip magic 1F 8B");

	/* Decompress half -- fresh region to reclaim the first stream's
	 * zlib state (arena-bump means no real free, and we'd exhaust
	 * the 1 MB otherwise). */
	jsval_region_init(&region, storage, storage_len);
	GENERATED_TEST_ASSERT(
			...
			jsval_readable_stream_get_reader(&region, out_readable, &reader)
				== 0,
			SUITE, CASE_NAME,
			"compressed readable + decompression stream + pipeThrough");
	...
	GENERATED_TEST_ASSERT(
			...
			SUITE, CASE_NAME,
			"decompressed output must match original input");
