...
#define CASE_NAME "jsmx/text-encoder-stream-validator-parity"
...
main(void)
{
	...
	uint8_t out[64];
	size_t out_len = 0;
	static const uint8_t valid_in[] = { 0x41, 0x42, 0xCE, 0xB1 };
	static const uint8_t expected[] = {
		0x41, 0x42, 0xCE, 0xB1, 0xEF, 0xBF, 0xBD,
	};

	jsval_region_init(&region, storage, sizeof(storage));

	GENERATED_TEST_ASSERT(
			jsval_text_encoder_stream_new(&region, &stream) == 0 &&
			...
			jsval_readable_stream_get_reader(&region, readable, &reader)
				== 0,
			SUITE, CASE_NAME, "encoder + writer + reader setup");

	GENERATED_TEST_ASSERT(
			jsval_writable_stream_writer_write(&region, writer, valid_in,
			...
			jsval_writable_stream_writer_close(&region, writer, &pcl)
				== 0,
			SUITE, CASE_NAME, "valid + invalid writes + close");
	...
	GENERATED_TEST_ASSERT(
			...
			SUITE, CASE_NAME,
			"valid passthrough + U+FFFD replacement");
