...
#define CASE_NAME "jsmx/text-decoder-stream-valid-full-parity"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(
			jsval_writable_stream_writer_write(&region, writer, input,
				sizeof(input), &pw) == 0 &&
			jsval_writable_stream_writer_close(&region, writer, &pcl)
				== 0,
			SUITE, CASE_NAME, "single write + close");
	...
	GENERATED_TEST_ASSERT(
			...
			SUITE, CASE_NAME, "byte-identical round-trip");
