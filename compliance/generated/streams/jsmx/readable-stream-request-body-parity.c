...
#define CASE_NAME "jsmx/readable-stream-request-body-parity"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(
			jsval_request_body(&region, request, &stream) == 0 &&
	...
	GENERATED_TEST_ASSERT(
			jsval_request_body_used(&region, request, &used) == 0 &&
			used == 1,
			SUITE, CASE_NAME, "bodyUsed true after .body (Phase 1D)");

	/* text() on a used Request rejects with TypeError. */
	GENERATED_TEST_ASSERT(
			jsval_request_text(&region, request, &promise) == 0 &&
