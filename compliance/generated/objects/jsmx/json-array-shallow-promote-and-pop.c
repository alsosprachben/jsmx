...
#define CASE_NAME "jsmx/json-array-shallow-promote-and-pop"
...
main(void)
{
	 ...
	 * compliance/js/objects/jsmx/json-array-shallow-promote-and-pop.js
	 *
	 ...
	 * promotion, then shallow-promotes exactly once before the dense pop.
	 */
	...
	GENERATED_TEST_ASSERT(jsval_array_get(&region, array, 2, &got) == 0,
			SUITE, CASE_NAME, "failed to read parsed arr[2]");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, got, jsval_number(6.0)) == 1,
			SUITE, CASE_NAME, "expected parsed arr[2] to read back as 6");
	errno = 0;
	GENERATED_TEST_ASSERT(jsval_array_pop(&region, array, &got) < 0,
			SUITE, CASE_NAME, "expected JSON-backed pop to fail before promotion");
	GENERATED_TEST_ASSERT(errno == ENOTSUP, SUITE, CASE_NAME,
			"expected JSON-backed pop to fail with ENOTSUP");
	...
	GENERATED_TEST_ASSERT(jsval_array_pop(&region, array, &got) == 0,
			SUITE, CASE_NAME, "failed to pop promoted dense tail");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, got, jsval_number(6.0)) == 1,
			SUITE, CASE_NAME, "expected promoted pop to return 6");
	GENERATED_TEST_ASSERT(expect_json(&region, array, "[4,5]")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected emitted JSON after shallow-promoted pop");
