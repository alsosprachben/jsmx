...
#define CASE_NAME "jsmx/array-splice-dense-delete"
...
main(void)
{
	 ...
	 * compliance/js/objects/jsmx/array-splice-dense-delete.js
	 *
	 * This idiomatic flattened translation exercises delete-only middle edits
	 * through jsval_array_splice_dense().
	 */
	...
	GENERATED_TEST_ASSERT(jsval_array_splice_dense(&region, array, 1, 2, NULL, 0,
			&removed) == 0, SUITE, CASE_NAME,
			"failed to perform delete-only dense splice");
	GENERATED_TEST_ASSERT(expect_json(&region, array, "[1,4]")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"expected dense delete-only splice to compact the remaining suffix");
	GENERATED_TEST_ASSERT(expect_json(&region, removed, "[2,3]")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"expected dense delete-only splice to return the removed middle segment");
