...
#define CASE_NAME "jsmx/array-pop-empty-undefined"
...
main(void)
{
	 ...
	 * compliance/js/objects/jsmx/array-pop-empty-undefined.js
	 *
	 * This idiomatic flattened translation exercises empty pop plus pops from
	 * dense undefined tail slots created by an explicit length grow.
	 */
	...
	GENERATED_TEST_ASSERT(jsval_array_pop(&region, array, &got) == 0,
			SUITE, CASE_NAME, "failed to pop empty array");
	GENERATED_TEST_ASSERT(got.kind == JSVAL_KIND_UNDEFINED, SUITE, CASE_NAME,
			"expected empty pop to return undefined");
	GENERATED_TEST_ASSERT(jsval_array_set_length(&region, array, 2) == 0,
			SUITE, CASE_NAME, "failed to grow array length");
	GENERATED_TEST_ASSERT(jsval_array_pop(&region, array, &got) == 0,
			SUITE, CASE_NAME, "failed to pop grown undefined tail");
	GENERATED_TEST_ASSERT(got.kind == JSVAL_KIND_UNDEFINED, SUITE, CASE_NAME,
			"expected grown tail pop to return undefined");
	GENERATED_TEST_ASSERT(jsval_array_pop(&region, array, &got) == 0,
			SUITE, CASE_NAME, "failed to pop final undefined slot");
	GENERATED_TEST_ASSERT(got.kind == JSVAL_KIND_UNDEFINED, SUITE, CASE_NAME,
			"expected final pop to return undefined");
	GENERATED_TEST_ASSERT(jsval_array_length(&region, array) == 0,
			SUITE, CASE_NAME, "expected array to end empty after repeated pops");
	GENERATED_TEST_ASSERT(expect_json(&region, array, "[]")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected emitted JSON after repeated undefined pops");
