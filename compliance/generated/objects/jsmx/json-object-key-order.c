...
#define CASE_NAME "jsmx/json-object-key-order"
...
main(void)
{
	 ...
	 * compliance/js/objects/jsmx/json-object-key-order.js
	 *
	 * This idiomatic flattened translation exercises ordered key access over
	...
	for (i = 0; i < len; i++) {
		GENERATED_TEST_ASSERT(jsval_object_key_at(&region, object, i, &key) == 0,
				SUITE, CASE_NAME, "failed to read JSON-backed key");
		GENERATED_TEST_ASSERT(jsval_array_push(&region, keys, key) == 0,
				SUITE, CASE_NAME, "failed to append JSON-backed key");
	}
