...
#define CASE_NAME "jsmx/array-shift-observable"
...
main(void)
{
	 ...
	 * compliance/js/objects/jsmx/array-shift-observable.js
	 *
	 * This idiomatic flattened translation exercises dense native array front
	 * mutation through shift, including the push-then-shift round-trip.
	 */
	...
	GENERATED_TEST_ASSERT(jsval_array_shift(&region, array, &got) == 0,
			SUITE, CASE_NAME, "failed to shift initial dense head");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, got, jsval_number(1.0)) == 1,
			SUITE, CASE_NAME, "expected first shift to return 1");
	...
	GENERATED_TEST_ASSERT(jsval_array_shift(&region, array, &got) == 0,
			SUITE, CASE_NAME, "failed to shift compacted dense head");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, got, jsval_number(2.0)) == 1,
			SUITE, CASE_NAME, "expected second shift to return 2");
	GENERATED_TEST_ASSERT(jsval_array_length(&region, array) == 1,
			SUITE, CASE_NAME, "expected array length to shrink back to 1");
	GENERATED_TEST_ASSERT(expect_json(&region, array, "[9]")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected emitted JSON after dense shift round-trip");
