...
#define CASE_NAME "jsmx/json-array-shallow-promote-and-unshift"
...
main(void)
{
	 ...
	 * compliance/js/objects/jsmx/json-array-shallow-promote-and-unshift.js
	 *
	 ...
	 * promotion, then shallow-promotes exactly once before the dense unshift.
	 */
	...
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, got, jsval_number(5.0)) == 1,
			SUITE, CASE_NAME, "expected parsed arr[1] to read back as 5");
	errno = 0;
	GENERATED_TEST_ASSERT(jsval_array_unshift(&region, array, jsval_number(7.0))
			< 0, SUITE, CASE_NAME,
			"expected JSON-backed unshift to fail before promotion");
	GENERATED_TEST_ASSERT(errno == ENOTSUP, SUITE, CASE_NAME,
			"expected JSON-backed unshift to fail with ENOTSUP");
	...
	GENERATED_TEST_ASSERT(jsval_array_unshift(&region, array, jsval_number(7.0))
			== 0, SUITE, CASE_NAME,
			"failed to unshift promoted dense head");
	GENERATED_TEST_ASSERT(expect_json(&region, array, "[7,4,5]")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected emitted JSON after shallow-promoted unshift");
