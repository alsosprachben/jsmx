...
#define CASE_NAME "jsmx/json-array-shallow-promote-and-update"
...
main(void)
{
	static const uint8_t input[] = "[1,2]";
	static const char expected[] = "[7,2,3]";
	 ...
	 * compliance/js/objects/jsmx/json-array-shallow-promote-and-update.js
	 *
	 * This idiomatic flattened translation keeps the parsed array JSON-backed
	 * for the initial read, then shallow-promotes it with the exact final
	 * dense capacity required for the planned overwrite and append.
	 */
	...
	GENERATED_TEST_ASSERT(jsval_array_get(&region, array, 1, &got) == 0,
			SUITE, CASE_NAME, "failed to read parsed arr[1]");
	GENERATED_TEST_ASSERT(jsval_strict_eq(&region, got, jsval_number(2.0)) == 1,
			SUITE, CASE_NAME, "expected parsed arr[1] to read back as 2");
	...
	GENERATED_TEST_ASSERT(region.used == before_used + bytes, SUITE, CASE_NAME,
			"shallow array promotion consumed an unexpected number of bytes");
	...
	GENERATED_TEST_ASSERT(jsval_array_set(&region, array, 0, jsval_number(7.0))
			== 0, SUITE, CASE_NAME, "failed to overwrite arr[0]");
	GENERATED_TEST_ASSERT(jsval_array_push(&region, array, jsval_number(3.0))
			== 0, SUITE, CASE_NAME, "failed to append arr[2]");

	GENERATED_TEST_ASSERT(expect_json(&region, array, expected)
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected emitted JSON after shallow-promoted array update");
