...
#define CASE_NAME "jsmx/crypto-subtle-ed25519-errors"
...
main(void)
{
	...
	jsval_t bad_raw_promise;
	jsval_t bad_raw_input;
	jsval_t bad_raw_buffer;
	jsval_t bad_jwk;
	jsval_t bad_jwk_promise;
	jsval_t value;
	jsval_t result;
	...
	GENERATED_TEST_ASSERT(jsval_object_get_utf8(&region, pair,
			...
			CASE_NAME, "expected Ed25519 pair accessors");
	...
	GENERATED_TEST_ASSERT(jsval_promise_result(&region, sign_promise,
			...
			"expected Ed25519 sign result");

	/* Sign with a public key -> InvalidAccessError. */
	GENERATED_TEST_ASSERT(jsval_subtle_crypto_sign(&region, subtle_value,
			algorithm, public_key, data_buffer, &bad_sign_promise) == 0,
			SUITE, CASE_NAME, "failed to call Ed25519 sign with public key");
	GENERATED_TEST_ASSERT(jsval_promise_result(&region, bad_sign_promise,
			&result) == 0, SUITE, CASE_NAME,
			"failed to read Ed25519 bad-sign result");
	GENERATED_TEST_ASSERT(expect_dom_exception_name(&region, result,
			"InvalidAccessError", "sign with public key")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected Ed25519 bad-sign rejection");

	/* Verify with a private key -> InvalidAccessError. */
	GENERATED_TEST_ASSERT(jsval_subtle_crypto_verify(&region, subtle_value,
			algorithm, private_key, signature, data_buffer,
			&bad_verify_promise) == 0, SUITE, CASE_NAME,
			"failed to call Ed25519 verify with private key");
	GENERATED_TEST_ASSERT(jsval_promise_result(&region, bad_verify_promise,
			&result) == 0, SUITE, CASE_NAME,
			"failed to read Ed25519 bad-verify result");
	GENERATED_TEST_ASSERT(expect_dom_exception_name(&region, result,
			"InvalidAccessError", "verify with private key")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected Ed25519 bad-verify rejection");

	/* Raw import with wrong length (16 instead of 32) -> DataError. */
	GENERATED_TEST_ASSERT(jsval_typed_array_new(&region,
			JSVAL_TYPED_ARRAY_UINT8, 16, &bad_raw_input) == 0, SUITE,
			CASE_NAME, "failed to allocate bad raw input");
	GENERATED_TEST_ASSERT(jsval_typed_array_buffer(&region, bad_raw_input,
			&bad_raw_buffer) == 0, SUITE, CASE_NAME,
			"failed to read bad raw buffer");
	GENERATED_TEST_ASSERT(jsval_subtle_crypto_import_key(&region, subtle_value,
			raw_format, bad_raw_buffer, algorithm, 1, verify_only_usages,
			&bad_raw_promise) == 0, SUITE, CASE_NAME,
			"failed to call Ed25519 importKey(bad raw)");
	GENERATED_TEST_ASSERT(jsval_promise_result(&region, bad_raw_promise,
			&result) == 0, SUITE, CASE_NAME,
			"failed to read Ed25519 bad-raw result");
	GENERATED_TEST_ASSERT(expect_dom_exception_name(&region, result,
			"DataError", "importKey bad raw length")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected Ed25519 bad-raw rejection");
	...
	GENERATED_TEST_ASSERT(make_string(&region, "OKP", &value) == 0
			&& jsval_object_set_utf8(&region, bad_jwk, (const uint8_t *)"kty",
				3, value) == 0
			&& make_string(&region, "X25519", &value) == 0
			&& jsval_object_set_utf8(&region, bad_jwk, (const uint8_t *)"crv",
				3, value) == 0
			&& make_string(&region,
				"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", &value) == 0
			&& jsval_object_set_utf8(&region, bad_jwk, (const uint8_t *)"x",
				1, value) == 0, SUITE, CASE_NAME,
			"failed to populate malformed JWK");
	GENERATED_TEST_ASSERT(jsval_subtle_crypto_import_key(&region, subtle_value,
			jwk_format, bad_jwk, algorithm, 1, verify_only_usages,
			&bad_jwk_promise) == 0, SUITE, CASE_NAME,
			"failed to call Ed25519 importKey(bad JWK)");
	GENERATED_TEST_ASSERT(jsval_promise_result(&region, bad_jwk_promise,
			&result) == 0, SUITE, CASE_NAME,
			"failed to read Ed25519 bad JWK result");
	GENERATED_TEST_ASSERT(expect_dom_exception_name(&region, result,
			"DataError", "importKey bad JWK crv")
			== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected Ed25519 bad-JWK rejection");
