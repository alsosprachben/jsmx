...
#define CASE_NAME "jsmx/crypto-subtle-rsa-pss-parity"
...
main(void)
{
	...
	GENERATED_TEST_ASSERT(jsval_object_get_utf8(&region, result,
			...
			&& expect_string(&region, prop, "RSA-PSS", "algorithm name")
				== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected RSA-PSS algorithm name");
	...
	GENERATED_TEST_ASSERT(jsval_subtle_crypto_verify(&region, subtle_value,
			...
			"failed to call RSA-PSS verify");
	memset(&error, 0, sizeof(error));
	GENERATED_TEST_ASSERT(jsval_microtask_drain(&region, &error) == 0, SUITE,
			CASE_NAME, "failed to drain RSA-PSS verify");
	...
	GENERATED_TEST_ASSERT(jsval_subtle_crypto_export_key(&region, subtle_value,
			...
			CASE_NAME, "failed to call RSA-PSS exportKey(public jwk)");
	memset(&error, 0, sizeof(error));
	GENERATED_TEST_ASSERT(jsval_microtask_drain(&region, &error) == 0, SUITE,
			CASE_NAME, "failed to drain RSA-PSS exportKey(public jwk)");
	GENERATED_TEST_ASSERT(jsval_promise_result(&region, public_export_promise,
			...
			CASE_NAME, "expected RSA-PSS public JWK");
	GENERATED_TEST_ASSERT(jsval_object_get_utf8(&region, jwk_public,
			(const uint8_t *)"alg", 3, &prop) == 0
			&& expect_string(&region, prop, "PS256", "public alg")
				== GENERATED_TEST_PASS, SUITE, CASE_NAME,
			"unexpected RSA-PSS public JWK alg");

	GENERATED_TEST_ASSERT(jsval_subtle_crypto_export_key(&region, subtle_value,
			jwk_format, private_key, &private_export_promise) == 0, SUITE,
			CASE_NAME, "failed to call RSA-PSS exportKey(private jwk)");
	memset(&error, 0, sizeof(error));
	GENERATED_TEST_ASSERT(jsval_microtask_drain(&region, &error) == 0, SUITE,
			CASE_NAME, "failed to drain RSA-PSS exportKey(private jwk)");
	GENERATED_TEST_ASSERT(jsval_promise_result(&region, private_export_promise,
			...
			CASE_NAME, "expected RSA-PSS private JWK");
	GENERATED_TEST_ASSERT(jsval_object_get_utf8(&region, jwk_private,
			...
			"expected RSA-PSS private JWK d field");
	...
	/* SPKI / PKCS#8 round-trip on the same key pair. */
	{
		...
		GENERATED_TEST_ASSERT(jsval_subtle_crypto_export_key(&region,
				...
				"failed to call RSA-PSS exportKey(spki)");
		memset(&error, 0, sizeof(error));
		GENERATED_TEST_ASSERT(jsval_microtask_drain(&region, &error) == 0,
				SUITE, CASE_NAME, "failed to drain RSA-PSS exportKey(spki)");
		GENERATED_TEST_ASSERT(jsval_promise_result(&region,
				...
				CASE_NAME, "expected RSA-PSS SPKI ArrayBuffer");

		GENERATED_TEST_ASSERT(jsval_subtle_crypto_export_key(&region,
				subtle_value, pkcs8_format, private_key,
				&pkcs8_export_promise) == 0, SUITE, CASE_NAME,
				"failed to call RSA-PSS exportKey(pkcs8)");
		memset(&error, 0, sizeof(error));
		GENERATED_TEST_ASSERT(jsval_microtask_drain(&region, &error) == 0,
				SUITE, CASE_NAME, "failed to drain RSA-PSS exportKey(pkcs8)");
		GENERATED_TEST_ASSERT(jsval_promise_result(&region,
				pkcs8_export_promise, &pkcs8_bytes) == 0
				&& pkcs8_bytes.kind == JSVAL_KIND_ARRAY_BUFFER, SUITE,
				CASE_NAME, "expected RSA-PSS PKCS#8 ArrayBuffer");
		...
		GENERATED_TEST_ASSERT(jsval_subtle_crypto_import_key(&region,
				...
				CASE_NAME, "failed to call RSA-PSS importKey(spki)");
		memset(&error, 0, sizeof(error));
		GENERATED_TEST_ASSERT(jsval_microtask_drain(&region, &error) == 0,
				SUITE, CASE_NAME, "failed to drain RSA-PSS importKey(spki)");
		GENERATED_TEST_ASSERT(jsval_promise_result(&region,
				spki_reimport_promise, &spki_reimported) == 0
				&& spki_reimported.kind == JSVAL_KIND_CRYPTO_KEY, SUITE,
				CASE_NAME, "expected RSA-PSS SPKI reimport");

		GENERATED_TEST_ASSERT(jsval_subtle_crypto_verify(&region, subtle_value,
				op_algorithm, spki_reimported, signature, data_buffer,
				&spki_reverify_promise) == 0, SUITE, CASE_NAME,
				"failed to call RSA-PSS SPKI reverify");
		memset(&error, 0, sizeof(error));
		GENERATED_TEST_ASSERT(jsval_microtask_drain(&region, &error) == 0,
				SUITE, CASE_NAME, "failed to drain RSA-PSS SPKI reverify");
		GENERATED_TEST_ASSERT(jsval_promise_result(&region,
				spki_reverify_promise, &result) == 0
				&& result.kind == JSVAL_KIND_BOOL
				&& result.as.boolean == 1, SUITE, CASE_NAME,
				"expected RSA-PSS SPKI reverify true");
	}
