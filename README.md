...
- `jsregex.c`, `jsregex.h`, `jsmx_config.h`: optional backend-gated regex layer; the first backend is PCRE2 and the current JS-facing slice covers regex-backed `search`, `exec`, `match`, `matchAll`, `RegExp.prototype.test`, semantic `source`, and core regex state helpers
