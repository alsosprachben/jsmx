...
- `jsregex.c`, `jsregex.h`, `jsmx_config.h`: optional backend-gated regex layer; the first backend is PCRE2 and the current JS-facing slice covers regex-backed `search`, `exec`, `match`, `RegExp.prototype.test`, semantic `source`, and core regex state helpers
...
- `jsmethod.c`, `jsmethod.h`: thin JS-method helpers layered over `jsstr`/`unicode` for receiver coercion, Symbol rejection, typed method errors, and `String.prototype.normalize`, `concat`, `replace`, case-conversion, trim/repeat plus the legacy trim aliases, padding, non-regex search/membership including `split`, the first regex-backed `search` entrypoint, accessor, substring-extraction including `substr`, and `toWellFormed`-style behavior, including exact `normalize` sizing for deterministic values
